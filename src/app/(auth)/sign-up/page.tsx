...
import { useSignUpMutation } from "@/lib/tanstack-query/mutations/use-auth";
...
function SignUpPage() {
  ...
  const onSubmit = (values: z.infer<typeof registerSchema>) => {
    signUpMutation.mutate({ ...values }, {
      onSuccess: (_data, { email, username }) => {
        router.push(`/verify?email=${email}&username=${username}`);
      }
    });
  };
      ...
      <div className="bg-[#F4F4F0] h-screen w-full lg:col-span-3 overflow-y-auto">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4 p-8 lg:p-16">
            ...
            <Button type="submit" size={'lg'} variant={'elevated'} className="bg-black text-white hover:bg-pink-400 hover:text-primary">Create Account</Button>
          </form>
        </Form>
      </div>
      <div
